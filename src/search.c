...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = log(depth) * log(moves) / 2.2621 + 0.2521;
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = 1.2146 + 0.3119 * depth * depth;
    LMP[1][depth] = 2.4379 + 0.9500 * depth * depth;

    STATIC_PRUNE[0][depth] = -13.4436 * depth * depth; // quiet move cutoff
    STATIC_PRUNE[1][depth] = -99.4148 * depth;        // capture cutoff
  }
}
...
void Search(ThreadData* thread) {
  ...
  while (++thread->depth < MAX_SEARCH_PLY) {
    ...
    if (Limits.timeset && thread->depth >= 5 && !Threads.stopOnPonderHit) {
      ...
      double stabilityFactor = 1.2833 - 0.0474 * searchStability;
      ...
      double scoreChangeFactor = 0.0917 +                                           //
                                 0.0284 * searchScoreDiff * (searchScoreDiff > 0) + //
                                 0.0319 * prevScoreDiff * (prevScoreDiff > 0);
      scoreChangeFactor = Max(0.4788, Min(1.6114, scoreChangeFactor));

      uint64_t bestMoveNodes = thread->rootMoves[0].nodes;
      double pctNodesNotBest = 1.0 - (double) bestMoveNodes / thread->nodes;
      double nodeCountFactor = Max(0.5113, pctNodesNotBest * 2.1893 + 0.4813);
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if (depth <= 9 && !ss->skip && eval < TB_WIN_BOUND && eval >= beta &&
        eval - 67 * depth + 115 * (improving && !board->easyCapture) >= beta &&
        (!hashMove || GetHistory(ss, thread, hashMove) > 12123))
      return (eval + beta) / 2;

    // Razoring
    if (depth <= 5 && eval + 281 * depth <= alpha) {
    ...
    // threats)
    if (depth >= 5 && (ss - 1)->move != NULL_MOVE && !ss->skip && eval >= beta && HasNonPawn(board, board->stm) &&
        (ss->ply >= thread->nmpMinPly || board->stm != thread->npmColor)) {
      int R = 4 + 233 * depth / 1024 + Min(5 * (eval - beta) / 1024, 3) + !board->easyCapture;
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (bestScore > -TB_WIN_BOUND) {
      ...
      if (!IsCap(move) && PromoPT(move) != QUEEN) {
        ...
        if (!killerOrCounter && lmrDepth < 7 && history < -2795 * (depth - 1)) {
          skipQuiets = 1;
          continue;
        }

        if (!inCheck && lmrDepth < 9 && eval + 96 + 48 * lmrDepth <= alpha)
          skipQuiets = 1;
    ...
    if (!isRoot && ss->ply < thread->depth * 2) {
      ...
      if (depth >= 6 && move == hashMove && ttDepth >= depth - 3 && (ttBound & BOUND_LOWER) &&
        ...
        if (score < sBeta) {
          if (!isPV && score < sBeta - 18 && ss->de <= 7) {
            extension = 2;
            ss->de    = (ss - 1)->de + 1;
          } else {
            extension = 1;
