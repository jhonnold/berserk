...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = log(depth) * log(moves) / 1.9477 + 0.1995;
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = 1.7666 + 0.3496 * depth * depth;
    LMP[1][depth] = 1.7608 + 0.8729 * depth * depth;

    STATIC_PRUNE[0][depth] = -13.4979 * depth * depth; // quiet move cutoff
    STATIC_PRUNE[1][depth] = -125.9288 * depth;         // capture cutoff
  }
}
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if ((ss - 1)->reduction >= 1 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 85)
      depth--;

    // Reverse Futility Pruning
    // i.e. the static eval is so far above beta we prune
    if (depth <= 9 && !ss->skip && eval < TB_WIN_BOUND && eval >= beta &&
        eval - 91 * depth + 167 * (improving && !opponentHasEasyCapture) + 28 * opponentDeclining >= beta &&
        (!hashMove || GetHistory(ss, thread, hashMove) > 10973))
      return (eval + beta) / 2;

    // Razoring
    if (depth <= 5 && eval + 136 * depth <= alpha) {
      DecRlx(thread->nodes);
    ...
    if (depth >= 5 && (ss - 1)->move != NULL_MOVE && !ss->skip && !opponentHasEasyCapture && eval >= beta &&
      ...
      int R = 4 + 331 * depth / 1024 + Min(10 * (eval - beta) / 1024, 5);
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (bestScore > -TB_WIN_BOUND) {
      ...
      if (!IsCap(move) && PromoPT(move) != QUEEN) {
        ...
        if (!killerOrCounter && lmrDepth < 3 && history < -2782 * (depth - 1)) {
          skipQuiets = 1;
          continue;
        }

        if (!inCheck && lmrDepth < 10 && eval + 58 + 44 * lmrDepth <= alpha)
          skipQuiets = 1;
