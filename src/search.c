...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if (depth <= 8 && !ss->skip && eval < WINNING_ENDGAME &&
        eval - 65 * depth + 117 * (improving && !oppThreatPcs) >= beta && eval >= beta)
      return eval;

    // Razoring
    if (depth <= 4 && eval + 139 * depth <= alpha) {
    ...
    // threats)
    if (depth >= 5 && (ss - 1)->move != NULL_MOVE && !ss->skip && eval >= beta &&
        // weiss conditional
        HasNonPawn(board) > (depth > 12)) {
      int R = 4 + 438 * depth / 2048 + Min(3 * (eval - beta) / 2048, 6) + !oppThreatPcs;
    ...
    int probBeta = beta + 103 - 38 * improving;
    if (depth > 3 && abs(beta) < TB_WIN_BOUND && ss->ownThreat.pcs &&
