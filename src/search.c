...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = log(depth) * log(moves) / 2.1902 + 0.2529;
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = 1.2259 + 0.3103 * depth * depth;
    LMP[1][depth] = 2.3865 + 0.8889 * depth * depth;

    STATIC_PRUNE[0][depth] = -13.4763 * depth * depth; // quiet move cutoff
    STATIC_PRUNE[1][depth] = -98.4942 * depth;        // capture cutoff
  }
}
...
void Search(ThreadData* thread) {
  ...
  while (++thread->depth < MAX_SEARCH_PLY) {
    ...
    if (Limits.timeset && thread->depth >= 5 && !Threads.stopOnPonderHit) {
      ...
      double stabilityFactor = 1.3894 - 0.0464 * searchStability;
      ...
      double scoreChangeFactor = 0.0931 +                                           //
                                 0.0287 * searchScoreDiff * (searchScoreDiff > 0) + //
                                 0.0302 * prevScoreDiff * (prevScoreDiff > 0);
      scoreChangeFactor = Max(0.4418, Min(1.5204, scoreChangeFactor));

      uint64_t bestMoveNodes = thread->rootMoves[0].nodes;
      double pctNodesNotBest = 1.0 - (double) bestMoveNodes / thread->nodes;
      double nodeCountFactor = Max(0.5248, pctNodesNotBest * 2.2397 + 0.4854);
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if (depth <= 9 && !ss->skip && eval < TB_WIN_BOUND && eval >= beta &&
        eval - 68 * depth + 117 * (improving && !board->easyCapture) >= beta &&
        (!hashMove || GetHistory(ss, thread, hashMove) > 12934))
      return eval;

    // Razoring
    if (depth <= 6 && eval + 283 * depth <= alpha) {
    ...
    if (depth >= 5 && (ss - 1)->move != NULL_MOVE && !ss->skip && eval >= beta && HasNonPawn(board, board->stm) &&
      ...
      int R = 4 + 229 * depth / 1024 + Min(5 * (eval - beta) / 1024, 3) + !board->easyCapture;
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (bestScore > -TB_WIN_BOUND) {
      ...
      if (!IsCap(move) && PromoPT(move) != QUEEN) {
        ...
        if (!killerOrCounter && lmrDepth < 7 && history < -2704 * (depth - 1)) {
          skipQuiets = 1;
          continue;
        }

        if (!inCheck && lmrDepth < 9 && eval + 93 + 46 * lmrDepth <= alpha)
          skipQuiets = 1;
