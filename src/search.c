...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (inCheck) {
    ...
    if (ttHit) {
      ...
      eval = ss->staticEval = ClampEval(rawEval + GetPawnCorrection(board, thread) / 2 + GetContCorrection(ss) + GetKingAreaCorrection(board, thread) / 2);
  ...
  if (!inCheck && !IsCap(bestMove) && (bound & (bestScore >= ss->staticEval ? BOUND_LOWER : BOUND_UPPER))) {
    ...
    UpdateKingAreaCorrection(ss->staticEval, bestScore, depth, board, thread);
...
void SearchClearThread(ThreadData* thread) {
  ...
  memset(&thread->kingAreaCorrection, 0, sizeof(thread->kingAreaCorrection));
