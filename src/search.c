...
void InitPruningAndReductionTables() {
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = b0 + b1 * depth * depth;
    LMP[1][depth] = c0 + c1 * depth * depth;

    STATIC_PRUNE[0][depth] = -d0 * depth * depth; // quiet move cutoff
    STATIC_PRUNE[1][depth] = -e0 * depth; // capture cutoff
  }
}
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    // threats)
    if (depth >= k0 && (ss - 1)->move != NULL_MOVE && !ss->skip && !opponentHasEasyCapture && eval >= beta
        && HasNonPawn(board, board->stm) && (ss->ply >= thread->nmpMinPly || board->stm != thread->npmColor)) {
      int R = k1 + k2 * depth / 1024 + Min(k3 * (eval - beta) / 1024, k4);
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (!isRoot && ss->ply < thread->depth * 2) {
      ...
      if (depth >= o0 && move == hashMove && ttDepth >= depth - 3 && (ttBound & BOUND_LOWER) &&
          abs(ttScore) < TB_WIN_BOUND) {
        int sBeta  = Max(ttScore - o1 * depth / 8, -CHECKMATE);
        ...
        if (score < sBeta) {
          if (!isPV && score < sBeta - o4 && ss->de <= o3 && !IsCap(move)) {
            extension = 3;
            ss->de    = (ss - 1)->de + 1;
          } else if (!isPV && score < sBeta - o2 && ss->de <= o3) {
            extension = 2;
            ss->de    = (ss - 1)->de + 1;
          } else {
            extension = 1;
    ...
    if (score > bestScore) {
      ...
      if (score > alpha) {
        ...
        if (alpha < beta && score > -TB_WIN_BOUND)
          depth -= (depth >= 2 && depth <= r0);
      }

      // we're failing high
      if (alpha >= beta) {
        UpdateHistories(ss, thread, move, depth + (bestScore > beta + s0), quiets, numQuiets, captures, numCaptures);
        break;
      }
    }
  }
