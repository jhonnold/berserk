...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = 460 * log(depth) * log(moves) + 243;
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if ((ss - 1)->reduction >= 3018 && !opponentDeclining)
      depth++;

    if ((ss - 1)->reduction >= 1174 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 79)
      depth--;
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    R -= 5649 * history / 65536;                        // adjust reduction based on historical score
    R += 1092 * (IsCap(hashMove) || IsPromo(hashMove)); // increase reduction if hash move is noisy
    ...
    if (depth > 1 && legalMoves > 1 && !(isPV && IsCap(move))) {
      ...
      if (cutnode)
        R += 955 + 1028 * !IsCap(move);

      if (ttDepth >= depth)
        R -= 1089;

      // prevent dropping into QS, extending, or reducing all extensions
      int reduction = Min(newDepth, Max(R / 1024, 1));
      ss->reduction = R;
