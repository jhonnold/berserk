...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (inCheck) {
    ...
    if (ttHit) {
      ...
      eval = ss->staticEval = ClampEval(rawEval + GetPawnCorrection(board, thread) / 2 + GetContCorrection(ss, thread) +
                                        GetMajorCorrection(board, thread) / 2);
  ...
  if (!inCheck && !IsCap(bestMove) && (bound & (bestScore >= rawEval ? BOUND_LOWER : BOUND_UPPER))) {
    ...
    UpdateMajorCorrection(rawEval, bestScore, depth, board, thread);
...
void SearchClearThread(ThreadData* thread) {
  ...
  memset(&thread->majorCorrection, 0, sizeof(thread->majorCorrection));
