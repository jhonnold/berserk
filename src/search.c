...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = log(depth) * log(moves) / 2.0871 + 0.1901;
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = 1.6197 + 0.3469 * depth * depth;
    LMP[1][depth] = 1.7471 + 0.9425 * depth * depth;

    STATIC_PRUNE[0][depth] = -13.7728 * depth * depth; // quiet move cutoff
    STATIC_PRUNE[1][depth] = -116.8165 * depth;         // capture cutoff
  }
}
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if ((ss - 1)->reduction >= 1 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 83)
      depth--;

    // Reverse Futility Pruning
    // i.e. the static eval is so far above beta we prune
    if (depth <= 9 && !ss->skip && eval < TB_WIN_BOUND && eval >= beta &&
        eval - 85 * depth + 160 * (improving && !opponentHasEasyCapture) + 27 * opponentDeclining >= beta &&
        (!hashMove || GetHistory(ss, thread, hashMove) > 11430))
      return (eval + beta) / 2;

    // Razoring
    if (depth <= 5 && eval + 150 * depth <= alpha) {
      DecRlx(thread->nodes);
    ...
    if (depth >= 5 && (ss - 1)->move != NULL_MOVE && !ss->skip && !opponentHasEasyCapture && eval >= beta &&
      ...
      int R = 4 + 336 * depth / 1024 + Min(9 * (eval - beta) / 1024, 4);
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (bestScore > -TB_WIN_BOUND) {
      ...
      if (!IsCap(move) && PromoPT(move) != QUEEN) {
        ...
        if (!killerOrCounter && lmrDepth < 4 && history < -3041 * (depth - 1)) {
          skipQuiets = 1;
          continue;
        }

        if (!inCheck && lmrDepth < 10 && eval + 62 + 44 * lmrDepth <= alpha)
          skipQuiets = 1;
    ...
    if (!isRoot && ss->ply < thread->depth * 2) {
      ...
      if (depth >= 6 && move == hashMove && ttDepth >= depth - 3 && (ttBound & BOUND_LOWER) &&
        ...
        if (score < sBeta) {
          if (!isPV && score < sBeta - 43 && ss->de <= 6 && !IsCap(move)) {
            extension = 3;
            ss->de    = (ss - 1)->de + 1;
