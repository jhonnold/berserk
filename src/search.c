...
int Negamax(int alpha, int beta, int depth, ThreadData* thread, PV* pv) {
  ...
  if (!isRoot) {
    ...
    if (data->ply > MAX_SEARCH_PLY - 1)
      return Evaluate(board, thread) / 2;
  ...
  int eval;
  if (!skipMove) {
    eval = data->evals[data->ply] = board->checkers ? UNKNOWN : (ttHit ? tt->eval : Evaluate(board, thread) / 2);
  } else {
...
int Quiesce(int alpha, int beta, ThreadData* thread, PV* pv) {
  ...
  int eval = data->evals[data->ply] = board->checkers ? UNKNOWN : (ttHit ? tt->eval : Evaluate(board, thread) / 2);
  if (!ttHit)
    TTPut(board->zobrist, INT8_MIN, UNKNOWN, TT_UNKNOWN, NULL_MOVE, data->ply, eval);
  ...
  if (board->checkers)
    InitAllMoves(&moves, ttHit ? tt->move : NULL_MOVE, data);
  else
  ...
  while ((move = NextMove(&moves, board, !board->checkers))) {
    if (bestScore > -MATE_BOUND) {
      ...
      if (!Tactical(move) && SEE(board, move) < 0)
        continue;
    }
