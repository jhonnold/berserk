...
void InitPruningAndReductionTables() {
  for (int depth = 1; depth < MAX_SEARCH_PLY; depth++)
    for (int moves = 1; moves < 64; moves++)
      LMR[depth][moves] = log(depth) * log(moves) / 2.1835 + 0.2758;
  ...
  for (int depth = 0; depth < MAX_SEARCH_PLY; depth++) {
    ...
    // more
    LMP[0][depth] = 1.3434 + 0.3698 * depth * depth;
    LMP[1][depth] = 2.8163 + 0.9254 * depth * depth;

    STATIC_PRUNE[0][depth] = -15.1002 * depth * depth; // quiet move cutoff
    STATIC_PRUNE[1][depth] = -103.1287 * depth;        // capture cutoff
  }
}
...
void Search(ThreadData* thread) {
  ...
  while (++thread->depth < MAX_SEARCH_PLY) {
    ...
    if (Limits.timeset && thread->depth >= 5 && !Threads.stopOnPonderHit) {
      ...
      double stabilityFactor = 1.3329 - 0.0464 * searchStability;
      ...
      double scoreChangeFactor = 0.1063 +                                           //
                                 0.0298 * searchScoreDiff * (searchScoreDiff > 0) + //
                                 0.0287 * prevScoreDiff * (prevScoreDiff > 0);
      scoreChangeFactor = Max(0.4658, Min(1.4918, scoreChangeFactor));

      uint64_t bestMoveNodes = thread->rootMoves[0].nodes;
      double pctNodesNotBest = 1.0 - (double) bestMoveNodes / thread->nodes;
      double nodeCountFactor = Max(0.5259, pctNodesNotBest * 2.2286 + 0.4662);
...
int Negamax(int alpha, int beta, int depth, int cutnode, ThreadData* thread, PV* pv, SearchStack* ss) {
  ...
  if (!isPV && !inCheck) {
    ...
    if (depth <= 8 && !ss->skip && eval < TB_WIN_BOUND && eval >= beta &&
        eval - 76 * depth + 122 * (improving && !board->easyCapture) >= beta &&
        (!hashMove || GetHistory(ss, thread, hashMove) > 11847))
      return (eval + beta) / 2;

    // Razoring
    if (depth <= 6 && eval + 258 * depth <= alpha) {
    ...
    if (depth >= 4 && (ss - 1)->move != NULL_MOVE && !ss->skip && eval >= beta && HasNonPawn(board, board->stm) &&
      ...
      int R = 5 + 219 * depth / 1024 + Min(5 * (eval - beta) / 1024, 4) + !board->easyCapture;
  ...
  while ((move = NextMove(&mp, board, skipQuiets))) {
    ...
    if (bestScore > -TB_WIN_BOUND) {
      ...
      if (!IsCap(move) && PromoPT(move) != QUEEN) {
        ...
        if (!killerOrCounter && lmrDepth < 7 && history < -2893 * (depth - 1)) {
          skipQuiets = 1;
          continue;
        }

        if (!inCheck && lmrDepth < 10 && eval + 82 + 44 * lmrDepth <= alpha)
          skipQuiets = 1;
      ...
      if (depth >= 6 && move == hashMove && ttDepth >= depth - 3 && (ttBound & BOUND_LOWER) &&
        ...
        if (score < sBeta) {
          if (!isPV && score < sBeta - 50 && ss->de <= 6 && !IsCap(move)) {
            extension = 3;
            ss->de    = (ss - 1)->de + 1;
          } else if (!isPV && score < sBeta - 18 && ss->de <= 6) {
            extension = 2;
            ss->de    = (ss - 1)->de + 1;
          } else {
            extension = 1;
