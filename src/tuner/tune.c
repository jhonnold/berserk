...
void Tune() {
  ...
  for (int epoch = 1; epoch <= 100000; epoch++) {
    for (int i = 0; i < n / BATCH_SIZE; i++)
      UpdateAndTrain(i, positions, &weights);

    long now = GetTimeMS();

    double error = TotalTunedError(n, positions, &weights);
...
#pragma omp parallel for schedule(static) num_threads(THREADS) reduction(+ : e)
  for (int i = 0; i < n; i++) {
    ...
    double sigmoid = Sigmoid(positions[i].staticEval, K);
...
void UpdateParam(Param* p) {
  ...
  float g = p->g / BATCH_SIZE;
  p->M = BETA1 * p->M + (1.0 - BETA1) * g;
  p->V = BETA2 * p->V + (1.0 - BETA2) * g * g;
...
double TotalTunedError(int n, Position* positions, Weights* weights) {
...
double UpdateAndTrain(int n, Position* positions, Weights* weights) {
  ...
  for (int t = 0; t < THREADS; t++) {
    ...
    jobs[t].error = 0.0;
    jobs[t].n = t < THREADS - 1 ? chunk : (BATCH_SIZE - ((THREADS - 1) * chunk));
  ...
  for (int t = 0; t < THREADS; t++) {
    ...
    weights->passedPawnOutsideVKnight.mg.g += w->passedPawnOutsideVKnight.mg.g;
    weights->passedPawnOutsideVKnight.eg.g += w->passedPawnOutsideVKnight.eg.g;
    ...
    weights->complexPawnsBothSides.mg.g += w->complexPawnsBothSides.mg.g;
...
void* CalculateError(void* arg) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    double eval = EvaluateCoeffs(&positions[i], weights, gd);

    double sigmoid = Sigmoid(eval, K);
...
void* UpdateGradients(void* arg) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    double sigmoid = Sigmoid(actual, K);
...
void UpdateComplexityGradients(Position* position, double loss, Weights* weights, EvalGradientData* gd) {
  ...
  weights->complexPawnsBothSides.mg.g += position->coeffs.complexPawnsBothSides * egBase * sign;
...
void LoadPosition(Board* board, Position* position, ThreadData* thread) {
  ...
  position->stm = board->stm;

  Score eval = Evaluate(board, thread);
  position->staticEval = sideScalar[board->stm] * eval;
...
Position* LoadPositions(int* n, Weights* weights) {
  ...
  while (p < MAX_POSITIONS && fgets(buffer, 128, fp)) {
    ...
    if (!(board.pieces[WHITE_PAWN] | board.pieces[BLACK_PAWN]))
      continue;

    if (BitCount(board.occupancies[BOTH]) == 3 && (board.pieces[WHITE_PAWN] | board.pieces[BLACK_PAWN]))
      continue;

    LoadPosition(&board, &positions[p], thread);
    if (abs(positions[p].staticEval) > 1500)
      continue;
...
void InitPsqtWeights(Weights* weights) {
  for (int sq = 0; sq < 32; sq++) {
    for (int i = 0; i < 2; i++) {
      weights->psqt[PAWN][i][sq].mg.value = scoreMG(PAWN_PSQT[i][sq]);
      weights->psqt[PAWN][i][sq].eg.value = scoreEG(PAWN_PSQT[i][sq]);

      weights->psqt[KNIGHT][i][sq].mg.value = scoreMG(KNIGHT_PSQT[i][sq]);
      weights->psqt[KNIGHT][i][sq].eg.value = scoreEG(KNIGHT_PSQT[i][sq]);

      weights->psqt[BISHOP][i][sq].mg.value = scoreMG(BISHOP_PSQT[i][sq]);
      weights->psqt[BISHOP][i][sq].eg.value = scoreEG(BISHOP_PSQT[i][sq]);

      weights->psqt[ROOK][i][sq].mg.value = scoreMG(ROOK_PSQT[i][sq]);
      weights->psqt[ROOK][i][sq].eg.value = scoreEG(ROOK_PSQT[i][sq]);

      weights->psqt[QUEEN][i][sq].mg.value = scoreMG(QUEEN_PSQT[i][sq]);
      weights->psqt[QUEEN][i][sq].eg.value = scoreEG(QUEEN_PSQT[i][sq]);

      weights->psqt[KING][i][sq].mg.value = scoreMG(KING_PSQT[i][sq]);
      weights->psqt[KING][i][sq].eg.value = scoreEG(KING_PSQT[i][sq]);
    }
  }
}
...
void PrintWeights(Weights* weights, int epoch, double error) {
  ...
  PrintWeightArray(fp, weights->psqt[PAWN][0], 32, 4);
  fprintf(fp, "},{\n");
  PrintWeightArray(fp, weights->psqt[PAWN][1], 32, 4);
  fprintf(fp, "}};\n");

  fprintf(fp, "\nconst Score KNIGHT_PSQT[2][32] = {{\n");
  PrintWeightArray(fp, weights->psqt[KNIGHT][0], 32, 4);
  fprintf(fp, "},{\n");
  PrintWeightArray(fp, weights->psqt[KNIGHT][1], 32, 4);
  fprintf(fp, "}};\n");

  fprintf(fp, "\nconst Score BISHOP_PSQT[2][32] = {{\n");
  PrintWeightArray(fp, weights->psqt[BISHOP][0], 32, 4);
  fprintf(fp, "},{\n");
  PrintWeightArray(fp, weights->psqt[BISHOP][1], 32, 4);
  fprintf(fp, "}};\n");

  fprintf(fp, "\nconst Score ROOK_PSQT[2][32] = {{\n");
  PrintWeightArray(fp, weights->psqt[ROOK][0], 32, 4);
  fprintf(fp, "},{\n");
  PrintWeightArray(fp, weights->psqt[ROOK][1], 32, 4);
  fprintf(fp, "}};\n");

  fprintf(fp, "\nconst Score QUEEN_PSQT[2][32] = {{\n");
  PrintWeightArray(fp, weights->psqt[QUEEN][0], 32, 4);
  fprintf(fp, "},{\n");
  PrintWeightArray(fp, weights->psqt[QUEEN][1], 32, 4);
  fprintf(fp, "}};\n");

  fprintf(fp, "\nconst Score KING_PSQT[2][32] = {{\n");
  PrintWeightArray(fp, weights->psqt[KING][0], 32, 4);
  fprintf(fp, "},{\n");
  PrintWeightArray(fp, weights->psqt[KING][1], 32, 4);
  fprintf(fp, "}};\n");
