...
int StaticMaterialScore(int side, Board* board) {
  ...
  for (int p = PAWN[side]; p <= QUEEN[side]; p += 2)
    s += bits(board->pieces[p]) * MATERIAL_VALUES[PIECE_TYPE[p]];
...
int EvaluateMaterialOnlyEndgame(Board* board) {
  ...
  if (!whiteMaterial || !blackMaterial) {
    ...
    if (ssPieceCount <= wsPieceCount + 1 && materialDiff <= MATERIAL_VALUES[BISHOP_TYPE]) {
      if (ssMaterial >= MATERIAL_VALUES[ROOK_TYPE] + MATERIAL_VALUES[BISHOP_TYPE])
        eval /= 4;
      else
        eval /= 8;
    }
