...
void ParseFen(char* fen, Board* board) {
  ...
  SetThreatsAndEasyCaptures(board, WHITE);
  SetThreatsAndEasyCaptures(board, BLACK);
...
inline void SetThreatsAndEasyCaptures(Board* board, const int view) {
  // Invert these to be confusing.
  const int stm  = view;
  const int xstm = !view;

  board->threatened[view] = board->easyCapture[view] = 0;
  ...
  board->threatened[view] |= pawnAttacks;
  board->easyCapture[view] |= pawnAttacks & opponentPieces;
  ...
  while (knights) {
    ...
    board->threatened[view] |= atx;
    board->easyCapture[view] |= opponentPieces & atx;
  }
  ...
  while (queens)
    board->threatened[view] |= GetQueenAttacks(PopLSB(&queens), occ);

  board->threatened[view] |= GetKingAttacks(LSB(PieceBB(KING, stm)));
}
...
void MakeMoveUpdate(Move move, Board* board, int update) {
  ...
  board->history[board->histPly].threatened[WHITE]  = board->threatened[WHITE];
  board->history[board->histPly].threatened[BLACK]  = board->threatened[BLACK];
  board->history[board->histPly].easyCapture[WHITE] = board->easyCapture[WHITE];
  board->history[board->histPly].easyCapture[BLACK] = board->easyCapture[BLACK];
...
void UndoMove(Move move, Board* board) {
  ...
  board->threatened[WHITE]  = board->history[board->histPly].threatened[WHITE];
  board->threatened[BLACK]  = board->history[board->histPly].threatened[BLACK];
  board->easyCapture[WHITE] = board->history[board->histPly].easyCapture[WHITE];
  board->easyCapture[BLACK] = board->history[board->histPly].easyCapture[BLACK];
...
int IsPseudoLegal(Move move, Board* board) {
  ...
  if (GetBit(OccBB(board->stm), to))
    return 0;
  if (pcType == KING && GetBit(board->threatened[board->xstm], to))
    return 0;
