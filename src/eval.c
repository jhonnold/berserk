...
// clang-format off
const Score MATERIAL_VALUES[7] = { S(88, 153), S(433, 508), S(460, 536), S(579, 917), S(1556, 1659), S(   0,   0), S(   0,   0) };

const Score BISHOP_PAIR = S(8, 23);

const Score PAWN_PSQT[32] = {
 S(   4,  -1), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   1,  94), S(   0,  86), S(   3,  87), S(   6,  81),
 S(   6,  17), S(  11,  11), S(  21,  -4), S(  20, -11),
 S(   1,   6), S(   4,   4), S(   4,  -1), S(   9,  -1),
 S(  -1,   1), S(   1,   2), S(   4,  -1), S(   6,   3),
 S(   2,   0), S(   8,   1), S(   5,   2), S(   7,   5),
 S(   2,   1), S(   8,   1), S(   5,   3), S(   8,   9),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const Score KNIGHT_PSQT[32] = {
 S(  -4, -26), S(   5, -13), S(  21, -11), S(  25, -12),
 S(   4,  -6), S(   3,   6), S(  10,   1), S(   5,   3),
 S(  10, -10), S(   3,   4), S(   0,  10), S(   7,   2),
 S(  11,   1), S(   3,   3), S(   7,   2), S(  10,   4),
 S(   6,  11), S(   7,   4), S(   7,  12), S(   7,  13),
 S(  -1,   8), S(   2,   9), S(   2,  12), S(   6,  17),
 S(   3,  17), S(   1,   8), S(   1,  10), S(   4,  13),
 S(  -5,  13), S(   2,  12), S(   4,  10), S(   2,  17),
};

const Score BISHOP_PSQT[32] = {
 S(   3,   4), S(   6,   5), S(   6,   3), S(   2,   1),
 S(   2,   8), S(  -7,  14), S(  -1,  13), S(  -3,  12),
 S(   2,   8), S(  -1,   8), S(  -5,  12), S(   5,   3),
 S(  -5,  11), S(   3,   8), S(  -2,   9), S(   5,   9),
 S(   8,   6), S(  -2,   8), S(   3,   9), S(   6,   7),
 S(   4,   8), S(   8,  12), S(   5,  13), S(   4,  15),
 S(   8,   6), S(  10,   9), S(   7,   7), S(   2,  14),
 S(   7,   7), S(  10,   6), S(   5,  13), S(   2,  13),
};

const Score ROOK_PSQT[32] = {
 S(   4,  15), S(  -2,  17), S(  -9,  21), S( -10,  19),
 S(  14,   9), S(   6,  15), S(  10,  15), S(   7,  12),
 S(   6,  11), S(  20,  10), S(  19,  12), S(  18,   9),
 S(   6,  17), S(   8,  14), S(  11,  17), S(  12,  14),
 S(   2,  19), S(   5,  16), S(   3,  20), S(   8,  18),
 S(   4,  18), S(   8,  13), S(   9,  17), S(   8,  18),
 S(   0,  23), S(   6,  19), S(   8,  19), S(   9,  19),
 S(   7,  23), S(   7,  20), S(   8,  21), S(  10,  17),
};

const Score QUEEN_PSQT[32] = {
 S(  15,  41), S(  11,  46), S(   8,  51), S(  17,  43),
 S(  11,  37), S(   0,  52), S(   7,  55), S(   2,  53),
 S(  22,  29), S(  16,  40), S(  10,  53), S(  12,  50),
 S(  14,  43), S(  11,  53), S(  10,  49), S(   8,  56),
 S(  14,  43), S(  12,  49), S(  11,  48), S(   7,  59),
 S(  12,  40), S(  17,  34), S(  11,  46), S(  11,  42),
 S(  15,  25), S(  16,  23), S(  17,  25), S(  15,  32),
 S(  17,  19), S(  10,  28), S(   9,  28), S(  14,  22),
};

const Score KING_PSQT[32] = {
 S( -20, -77), S( -12, -11), S(  26,   3), S( -30,   4),
 S(  15,  -9), S(  37,  14), S(  32,  19), S(  30,  20),
 S(   2,  -3), S(  25,  12), S(  40,  17), S(  11,  20),
 S(   1,  -5), S(  30,  11), S(   9,  22), S(   0,  26),
 S( -33,   1), S(   9,  10), S(  24,  17), S(  13,  23),
 S(  -7,  -5), S(  10,   9), S(  14,  14), S(  12,  18),
 S(  -4, -12), S(  -4,   1), S(  -6,  11), S( -14,  16),
 S(  -3, -28), S(   0, -18), S(  -3,  -6), S(  -6, -10),
};

const Score KNIGHT_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(  -3,   2), S(   8,   6), S(  23,   1), S(  25,  14),
 S(   9,   3), S(  20,  12), S(  17,  19), S(  23,  17),
 S(   9,  -2), S(   9,  12), S(  13,   9), S(  10,  21),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const Score BISHOP_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   9,   1), S(  12,   9), S(  19,   7), S(  19,   4),
 S(   8,   3), S(  10,  13), S(  22,   4), S(  19,   5),
 S(  -3,  11), S(  14,   4), S(  10,   9), S(  16,  11),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const Score KNIGHT_MOBILITIES[9] = {
 S( -21, -23), S(  -9,  -3), S(  -2,   7), S(   2,  12),
 S(   6,  15), S(   9,  17), S(  11,  18), S(  14,  17),
 S(  17,  14),};

const Score BISHOP_MOBILITIES[14] = {
 S( -15, -16), S(  -7,  -6), S(  -2,  -1), S(   1,   5),
 S(   6,  10), S(   8,  13), S(   9,  14), S(  10,  16),
 S(  11,  16), S(  11,  18), S(  12,  16), S(  14,  18),
 S(  16,  23), S(  10,  20),};

const Score ROOK_MOBILITIES[15] = {
 S( -30, -74), S(  -3,  10), S(   2,  15), S(   5,  12),
 S(   4,  18), S(   4,  22), S(   3,  23), S(   4,  26),
 S(   5,  25), S(   7,  26), S(   9,  25), S(  10,  24),
 S(  11,  24), S(  13,  23), S(  12,  21),};

const Score QUEEN_MOBILITIES[28] = {
 S(-214, -37), S( -40, -12), S(   5,-137), S(   4, -15),
 S(   6,  15), S(   8,  24), S(   8,  36), S(  10,  39),
 S(  11,  44), S(  13,  44), S(  14,  45), S(  15,  46),
 S(  16,  48), S(  16,  48), S(  17,  49), S(  17,  49),
 S(  15,  49), S(  14,  54), S(  11,  52), S(  14,  50),
 S(  17,  41), S(  16,  39), S(  23,  19), S(  14,  25),
 S(   4,  20), S(   2,  11), S(  61, -81), S( -44,  30),
};

const Score KNIGHT_OUTPOST_REACHABLE = S(2, 8);

const Score BISHOP_OUTPOST_REACHABLE = S(2, 2);

const Score BISHOP_TRAPPED = S(-96, -80);

const Score ROOK_TRAPPED = S(-17, -16);

const Score ROOK_OPEN_FILE = S(8, 11);

const Score ROOK_SEMI_OPEN = S(0, 9);

const Score ROOK_OPPOSITE_KING = S(15, -14);

const Score ROOK_ADJACENT_KING = S(9, -29);
...
const Score CONNECTED_PAWN[8] = {
 S(   0,   0), S(  32,  13), S(  11,  15), S(   4,   6),
 S(   3,   2), S(   1,   1), S(   0,  -1), S(   0,   0),
};

const Score PASSED_PAWN[8] = {
 S(   0,   0), S(   2,  88), S( -17, 116), S( -10,  61),
 S(  -9,  30), S(  -2,  15), S(  -1,  12), S(   0,   0),
};
...
const Score PASSED_PAWN_ADVANCE_DEFENDED = S(6, 13);

const Score KNIGHT_THREATS[6] = { S(0, 8), S(1, 2), S(14, 13), S(22, 10), S(9, 7), S(-7, -8),};

const Score BISHOP_THREATS[6] = { S(2, 6), S(12, 16), S(0, 14), S(17, 5), S(17, 30), S(5, 36),};

const Score ROOK_THREATS[6] = { S(2, 10), S(8, 16), S(12, 18), S(-3, 14), S(23, -1), S(-11, 14),};

const Score KING_THREATS[6] = { S(8, 35), S(-12, 34), S(1, 16), S(6, 22), S(-11, 42), S(0, 0),};
