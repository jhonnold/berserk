...
// clang-format off
const Score MATERIAL_VALUES[7] = { S(77, 133), S(339, 402), S(356, 413), S(465, 716), S(1103, 1202), S(   0,   0), S(   0,   0) };

const Score BISHOP_PAIR = S(22, 53);

const Score PAWN_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(  50, 164), S(   1, 175), S(  37, 149), S(  76, 131),
 S(   4,  27), S(  -4,  22), S(  27, -14), S(  25, -27),
 S( -12,   3), S(   3, -10), S(  -3, -19), S(  16, -21),
 S( -16, -10), S( -10,  -9), S(  -1, -17), S(   4, -10),
 S( -14, -14), S(   8, -14), S(  -3, -12), S(   7,  -8),
 S( -12, -11), S(   5, -12), S(  -7, -10), S(   5,   2),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const Score KNIGHT_PSQT[32] = {
 S(-119, -62), S( -43, -27), S( -44, -15), S(   0, -21),
 S( -15, -11), S( -12,  14), S(  20,  -5), S(   5,  -1),
 S(  13, -18), S(   3,  -2), S(  -3,  12), S(  22,  -9),
 S(   9,   6), S(   0,  -4), S(  12,  -1), S(  19,  -3),
 S(  11,  27), S(  17,   1), S(  19,  21), S(  20,  23),
 S(  -8,  30), S(   6,  22), S(   2,  31), S(  17,  33),
 S(   3,  44), S(   8,  20), S(   5,  25), S(   9,  35),
 S( -22,  45), S(  -3,  32), S(   4,  27), S(   8,  39),
};

const Score BISHOP_PSQT[32] = {
 S( -27,  24), S( -12,  27), S( -33,  22), S( -68,  23),
 S(  -7,  17), S( -25,  36), S( -13,  31), S(  -7,  27),
 S(  -7,  23), S( -22,  26), S( -16,  29), S(  -2,  13),
 S( -23,  24), S(   3,  19), S(  -8,  18), S(   7,  17),
 S(  15,   6), S(  -8,  15), S(  -1,  20), S(   8,  14),
 S(   4,  15), S(  17,  23), S(   7,  29), S(   5,  34),
 S(  14,  14), S(  22,  19), S(  14,  15), S(   0,  34),
 S(  12,  12), S(  15,  16), S(   4,  31), S(   2,  28),
};

const Score ROOK_PSQT[32] = {
 S(   3,  34), S( -17,  43), S( -53,  60), S( -47,  58),
 S(  15,  21), S(   2,  34), S(  22,  28), S(  19,  23),
 S(   0,  29), S(  38,  25), S(  37,  26), S(  39,  18),
 S(  -6,  44), S(   8,  35), S(  23,  39), S(  21,  36),
 S( -13,  50), S(   3,  41), S(  -4,  51), S(   5,  51),
 S( -10,  49), S(   8,  35), S(   6,  45), S(   4,  49),
 S( -14,  64), S(   0,  52), S(   8,  51), S(  10,  52),
 S(   0,  73), S(   0,  61), S(   3,  61), S(   9,  53),
};

const Score QUEEN_PSQT[32] = {
 S(  66, 180), S(  48, 190), S(  77, 180), S(  71, 180),
 S(  53, 176), S(  27, 205), S(  51, 224), S(  49, 205),
 S(  80, 156), S(  71, 188), S(  60, 206), S(  71, 199),
 S(  65, 190), S(  68, 205), S(  62, 203), S(  54, 217),
 S(  71, 182), S(  69, 202), S(  65, 200), S(  56, 222),
 S(  70, 174), S(  78, 173), S(  65, 195), S(  66, 185),
 S(  76, 142), S(  79, 136), S(  80, 147), S(  77, 164),
 S(  79, 136), S(  65, 151), S(  64, 148), S(  72, 141),
};

const Score KING_PSQT[32] = {
 S(-174, -88), S( 136, -39), S( -39,  19), S( -39,   7),
 S( -18,  -4), S( 131,  26), S( 135,  38), S( 179,  26),
 S( -13,  -2), S( 185,  14), S( 183,  31), S(  94,  41),
 S( -35,   2), S( 180,  15), S( 118,  43), S(  63,  56),
 S(-106,  15), S(  81,  22), S( 133,  36), S( 101,  47),
 S( -38,   0), S(  42,  21), S(  77,  32), S(  89,  38),
 S( -39, -23), S( -29,  11), S( -25,  35), S( -42,  47),
 S( -41, -70), S( -34, -36), S( -33,  -2), S( -35,  -2),
};

const Score KNIGHT_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S( -17,  12), S(  24,  14), S(  49,  -3), S(  36,  34),
 S(  26,   3), S(  50,  28), S(  42,  35), S(  54,  32),
 S(  22,   0), S(  33,  29), S(  25,  22), S(  26,  39),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const Score BISHOP_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(  22,  -1), S(  42,   3), S(  55,   5), S(  50,   1),
 S(  18,   2), S(  31,  19), S(  45,   7), S(  47,   9),
 S(  -7,  26), S(  40,  11), S(  33,  12), S(  41,  20),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const Score KNIGHT_MOBILITIES[9] = {
 S( -45, -78), S( -23, -23), S( -10,  10), S(  -2,  22),
 S(   7,  30), S(  13,  38), S(  20,  40), S(  28,  36),
 S(  33,  28),};

const Score BISHOP_MOBILITIES[14] = {
 S( -36, -26), S( -18,  -6), S(  -7,   7), S(   1,  25),
 S(  11,  33), S(  16,  38), S(  20,  41), S(  21,  41),
 S(  24,  43), S(  24,  44), S(  30,  39), S(  48,  36),
 S(  48,  46), S(  40,  41),};

const Score ROOK_MOBILITIES[15] = {
 S( -44,-101), S( -18,  23), S(  -8,  40), S(   1,  28),
 S(  -2,  44), S(  -2,  52), S(  -4,  54), S(  -2,  60),
 S(   5,  57), S(   9,  55), S(  13,  53), S(  16,  46),
 S(  20,  44), S(  24,  40), S(  31,  31),};

const Score QUEEN_MOBILITIES[28] = {
 S(-281,-311), S( -76,   8), S(  50, -91), S(  43,  62),
 S(  48, 127), S(  50, 138), S(  48, 167), S(  52, 173),
 S(  53, 190), S(  56, 187), S(  58, 193), S(  60, 196),
 S(  60, 200), S(  62, 200), S(  63, 202), S(  66, 194),
 S(  61, 197), S(  61, 198), S(  60, 185), S(  62, 179),
 S(  73, 154), S(  91, 129), S( 104,  86), S( 108,  75),
 S( 124,  36), S( 208, -35), S( 147, -87), S(-339, 273),
};

const Score KNIGHT_OUTPOST_REACHABLE = S(5, 18);

const Score BISHOP_OUTPOST_REACHABLE = S(6, 3);

const Score BISHOP_TRAPPED = S(-127, -164);

const Score ROOK_TRAPPED = S(-43, -38);

const Score ROOK_OPEN_FILE = S(19, 24);

const Score ROOK_SEMI_OPEN = S(0, 29);

const Score ROOK_OPPOSITE_KING = S(32, -39);

const Score ROOK_ADJACENT_KING = S(17, -74);
...
const Score CONNECTED_PAWN[8] = {
 S(   0,   0), S(  96,  15), S(  30,  23), S(  12,  13),
 S(   6,   4), S(   2,   2), S(   0,  -2), S(   0,   0),
};

const Score PASSED_PAWN[8] = {
 S(   0,   0), S(  44, 158), S(  18, 208), S(   2, 117),
 S( -12,  60), S(  -8,  33), S(  -3,  29), S(   0,   0),
};

const Score PASSED_PAWN_EDGE_DISTANCE = S(-5, -8);

const Score PASSED_PAWN_KING_PROXIMITY = S(-6, 28);

const Score PASSED_PAWN_ADVANCE_DEFENDED = S(22, 23);

const Score KNIGHT_THREATS[6] = { S(0, 13), S(-3, 5), S(35, 18), S(67, -3), S(43, -48), S(0, 0),};

const Score BISHOP_THREATS[6] = { S(3, 12), S(28, 32), S(10, 8), S(43, 12), S(47, 71), S(0, 0),};

const Score ROOK_THREATS[6] = { S(5, 18), S(25, 34), S(30, 38), S(-7, 36), S(68, 13), S(0, 0),};

const Score KING_THREATS[6] = { S(56, 69), S(26, 81), S(28, 47), S(113, 29), S(0, 0), S(0, 0),};
