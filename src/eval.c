...
// clang-format off
const Score MATERIAL_VALUES[7] = { S(79, 139), S(350, 374), S(366, 383), S(531, 706), S(1074, 1361), S(   0,   0), S(   0,   0) };

const Score BISHOP_PAIR = S(25, 62);

const Score PAWN_PSQT[32] = {
 S(   0,  -1), S(   0,   0), S(   0,   0), S(   0,   0),
 S(  65, 151), S(  17, 174), S(  28, 149), S(  52, 123),
 S(  -1,  21), S( -11,  23), S(  15,  -1), S(  32, -29),
 S( -14,  -1), S(  -5, -13), S(  -6, -18), S(   6, -29),
 S( -17, -15), S( -17, -16), S(  -4, -24), S(   7, -26),
 S( -20, -20), S( -13, -22), S(  -7, -21), S(  -1, -18),
 S( -14, -14), S(   1, -18), S(  -4, -15), S(   1, -10),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const Score KNIGHT_PSQT[32] = {
 S(-143, -38), S( -91,   1), S( -84,  17), S( -26,  16),
 S( -30,   2), S( -19,  27), S(  30,  16), S(  21,  26),
 S(  27,  -8), S(  23,  15), S(  14,  36), S(  40,  26),
 S(  29,  23), S(  28,  30), S(  38,  33), S(  39,  41),
 S(  31,  29), S(  41,  23), S(  44,  37), S(  40,  46),
 S(   0,  26), S(  16,  24), S(  18,  32), S(  24,  46),
 S(  11,  36), S(  13,  23), S(  10,  28), S(  14,  33),
 S( -16,  38), S(   3,  24), S(   8,  30), S(  14,  38),
};

const Score BISHOP_PSQT[32] = {
 S( -53,  21), S(  -7,  40), S( -39,  44), S( -70,  51),
 S( -11,  32), S( -33,  30), S(   6,  44), S(  11,  40),
 S(  35,  31), S(  11,  40), S(  -7,  31), S(  23,  39),
 S(   1,  46), S(  38,  37), S(  29,  43), S(  32,  52),
 S(  43,  25), S(  27,  37), S(  34,  48), S(  38,  44),
 S(  20,  34), S(  38,  42), S(  19,  36), S(  26,  54),
 S(  31,  28), S(  26,  20), S(  33,  33), S(  19,  47),
 S(  15,  21), S(  32,  37), S(  17,  46), S(  20,  45),
};

const Score ROOK_PSQT[32] = {
 S(  -1,  58), S( -13,  62), S( -27,  73), S( -35,  73),
 S(  -1,  46), S(  -9,  59), S(   8,  55), S(  12,  47),
 S(  -7,  45), S(  33,  39), S(  24,  42), S(  29,  33),
 S( -11,  52), S(  11,  44), S(  23,  43), S(  15,  40),
 S( -14,  49), S(   9,  41), S(   0,  48), S(   3,  46),
 S( -14,  44), S(   8,  36), S(   8,  38), S(   4,  40),
 S( -18,  51), S(   1,  38), S(   6,  38), S(   9,  40),
 S(  -2,  40), S(  -2,  41), S(  -1,  42), S(   5,  35),
};

const Score QUEEN_PSQT[32] = {
 S(   6,  35), S(  11,  45), S(  46,  40), S(  34,  63),
 S( -10,  45), S( -40,  83), S( -16, 111), S( -15,  94),
 S(  22,  33), S(  15,  68), S(   0,  92), S(  10,  98),
 S(   9,  81), S(  12, 102), S(   9,  93), S( -10, 122),
 S(  14,  72), S(  17,  95), S(  11,  99), S(  -3, 122),
 S(   4,  63), S(  13,  67), S(   6,  90), S(   2,  88),
 S(   7,  40), S(  13,  26), S(  14,  43), S(  11,  60),
 S(  13,  22), S(  -2,  39), S(  -5,  41), S(   4,  34),
};

const Score KING_PSQT[32] = {
 S( -83, -96), S( 101, -32), S(  18,   6), S( -21, -11),
 S( -11,   3), S( 101,  41), S(  90,  41), S(  76,  23),
 S(  -2,  10), S( 140,  37), S( 120,  36), S(   9,  37),
 S( -43,   7), S(  99,  25), S(  12,  40), S( -29,  41),
 S( -94,   8), S(  21,  18), S(  28,  20), S( -22,  25),
 S( -15,  -5), S(  43,   8), S(  22,   9), S(  -1,  11),
 S(   5, -11), S(  10,  13), S( -13,   9), S( -44,   5),
 S(   1, -47), S(   7, -11), S(  -5, -17), S( -19, -38),
};

const Score KNIGHT_POST_PSQT[12] = {
 S( -29,  35), S(  15,  27), S(  41,  14), S(  33,  38),
 S(  15,  15), S(  36,  19), S(  28,  30), S(  44,  27),
 S(  13,   2), S(  26,  20), S(  13,  22), S(  14,  30),
};

const Score BISHOP_POST_PSQT[12] = {
 S(   5,  20), S(  30,  17), S(  60,  19), S(  52,   1),
 S(  11,  14), S(  17,  20), S(  28,   8), S(  34,   7),
 S( -15,  27), S(  26,  16), S(  18,   9), S(  31,  18),
};

const Score KNIGHT_MOBILITIES[9] = {
 S( -38, -70), S( -12, -14), S(   5,  18), S(  14,  30),
 S(  25,  40), S(  32,  47), S(  39,  49), S(  47,  47),
 S(  54,  39),};

const Score BISHOP_MOBILITIES[14] = {
 S( -16, -28), S(  -3,  -1), S(  11,  15), S(  17,  35),
 S(  25,  42), S(  31,  48), S(  33,  54), S(  35,  58),
 S(  37,  60), S(  38,  63), S(  45,  57), S(  64,  57),
 S(  71,  63), S(  62,  65),};

const Score ROOK_MOBILITIES[15] = {
 S( -66,-122), S( -19, -13), S(  -7,   7), S(   2,   8),
 S(  -1,  29), S(  -1,  38), S(  -5,  47), S(  -1,  48),
 S(   6,  48), S(   9,  53), S(  13,  55), S(  13,  58),
 S(  17,  62), S(  24,  63), S(  36,  57),};

const Score QUEEN_MOBILITIES[28] = {
 S(-741,-763), S(-161, -56), S(  -1,-217), S( -10, -48),
 S(  -7,  37), S(  -3,  44), S(  -5,  67), S(  -2,  86),
 S(   0,  99), S(   3, 102), S(   5, 105), S(   6, 110),
 S(   6, 114), S(   9, 113), S(  10, 117), S(  13, 107),
 S(  11, 107), S(  12, 107), S(   5,  99), S(  16,  83),
 S(  30,  55), S(  40,  38), S(  54,  -1), S(  41, -10),
 S(  58, -47), S( 197,-152), S( 279,-284), S(-358,  68),
};
...
const Score BISHOP_TRAPPED = S(-162, -110);
...
const Score CONNECTED_PAWN[8] = {
 S(   0,   0), S(  80,  37), S(  26,  28), S(  12,   9),
 S(   7,   2), S(   5,   1), S(   2,  -1), S(   0,   0),
};

const Score CANDIDATE_PASSER[8] = {
 S(   0,   0), S(   0,   0), S( -54, 201), S(   4,  46),
...
const Score KNIGHT_THREATS[6] = { S(0, 16), S(5, 7), S(39, 25), S(79, -6), S(47, -68), S(208, -2),};

const Score BISHOP_THREATS[6] = { S(2, 13), S(28, 31), S(11, 15), S(46, 6), S(49, 72), S(124, 84),};

const Score ROOK_THREATS[6] = { S(3, 24), S(33, 34), S(36, 42), S(3, 22), S(77, 13), S(186, 9),};

const Score KING_THREAT = S(2, 29);
...
const Score PAWN_SHELTER[4][8] = {
 { S(-26, 4), S(53, 22), S(-26, 42), S(-8, 10), S(3, -6), S(23, -17), S(23, -35), S(0, 0),},
 { S(-40, 1), S(-38, 56), S(-2, 24), S(-26, 14), S(-14, -1), S(20, -16), S(26, -24), S(0, 0),},
 { S(-20, -7), S(8, 42), S(-16, 23), S(-7, 4), S(-10, -2), S(-8, -3), S(24, -13), S(0, 0),},
 { S(-20, 5), S(25, 58), S(-53, 27), S(-20, 22), S(-14, 9), S(-11, 5), S(-10, 7), S(0, 0),},
};

const Score PAWN_STORM[4][8] = {
 { S(-28, -17), S(-14, -8), S(-13, -9), S(-22, 1), S(-44, 34), S(1, 138), S(157, 172), S(0, 0),},
 { S(-14, -8), S(-1, -10), S(0, -5), S(-11, 5), S(-24, 27), S(-113, 156), S(39, 212), S(0, 0),},
 { S(13, -9), S(22, -17), S(17, -9), S(9, -1), S(-15, 24), S(-126, 134), S(-95, 190), S(0, 0),},
 { S(4, -3), S(9, -21), S(14, -10), S(7, -15), S(-16, 9), S(-43, 97), S(-100, 170), S(0, 0),},
};

const Score BLOCKED_PAWN_STORM[8] = {
 S(-29, -6), S(46, -55), S(17, -34), S(17, -27), S(1, -17), S(-35, -24), S(0, 0), S(0, 0),
};

const Score KS_KING_FILE[4] = { S(12, -2), S(9, 0), S(1, 5), S(-30, -2),};
