...
// clang-format off
const Score MATERIAL_VALUES[7] = { S(86, 142), S(347, 368), S(358, 373), S(526, 687), S(1105, 1293), S(   0,   0), S(   0,   0) };

const Score BISHOP_PAIR = S(28, 59);

const Score PAWN_PSQT[32] = {
 S(  -1,  -1), S(   0,   0), S(   0,   0), S(   0,   0),
 S(  62, 149), S(  15, 169), S(  29, 140), S(  55, 111),
 S( -10,  27), S( -20,  27), S(   6,   4), S(  21, -26),
 S( -20,   1), S( -11, -12), S( -16, -19), S(   4, -33),
 S( -22, -14), S( -22, -17), S(  -8, -25), S(  -2, -27),
 S( -18, -18), S(  -5, -20), S(  -6, -21), S(   6, -20),
 S( -17, -12), S(   3, -17), S(  -8, -15), S(   6, -13),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const Score KNIGHT_PSQT[32] = {
 S(-139, -44), S( -91,  -5), S( -63,  10), S( -13,  11),
 S( -22,  -6), S( -15,  22), S(  36,  13), S(  23,  24),
 S(  26, -12), S(  24,  12), S(  13,  36), S(  37,  27),
 S(  29,  17), S(  26,  26), S(  35,  35), S(  39,  41),
 S(  31,  25), S(  40,  20), S(  42,  35), S(  40,  44),
 S(  -1,  22), S(  15,  23), S(  14,  30), S(  24,  45),
 S(  10,  32), S(  12,  20), S(   9,  27), S(  15,  33),
 S( -15,  33), S(   3,  18), S(   7,  26), S(  13,  33),
};

const Score BISHOP_PSQT[32] = {
 S( -35,  33), S( -14,  38), S( -55,  41), S( -69,  46),
 S( -12,  30), S( -16,  36), S(  -1,  38), S(   3,  32),
 S(  30,  28), S(   6,  34), S(  12,  36), S(  15,  30),
 S(  -2,  42), S(  37,  28), S(  23,  33), S(  29,  42),
 S(  44,  19), S(  22,  30), S(  30,  37), S(  32,  36),
 S(  17,  30), S(  35,  36), S(  27,  39), S(  22,  47),
 S(  29,  26), S(  34,  25), S(  29,  28), S(  14,  41),
 S(  24,  28), S(  31,  35), S(  15,  41), S(  15,  41),
};

const Score ROOK_PSQT[32] = {
 S(   4,  55), S(  -9,  59), S( -21,  69), S( -27,  69),
 S(   4,  44), S(  -8,  57), S(   9,  52), S(  14,  44),
 S(  -4,  43), S(  35,  37), S(  26,  41), S(  29,  31),
 S( -10,  51), S(  12,  40), S(  24,  40), S(  19,  37),
 S( -13,  46), S(  13,  38), S(   3,  47), S(   6,  43),
 S( -12,  42), S(  10,  32), S(   6,  36), S(   5,  37),
 S( -17,  48), S(   2,  36), S(   6,  36), S(  10,  38),
 S(  -1,  39), S(  -2,  38), S(   1,  41), S(   6,  34),
};

const Score QUEEN_PSQT[32] = {
 S(  10,  32), S(  13,  41), S(  46,  37), S(  36,  57),
 S( -11,  47), S( -41,  80), S( -19, 110), S( -20,  95),
 S(  24,  32), S(  18,  64), S(   2,  87), S(  10,  93),
 S(  10,  77), S(  15,  96), S(   9,  87), S(  -7, 118),
 S(  15,  67), S(  17,  93), S(   9,  95), S(  -2, 116),
 S(   5,  61), S(  13,  64), S(   2,  88), S(  -1,  84),
 S(   6,  39), S(  12,  23), S(  14,  39), S(   9,  54),
 S(  12,  19), S(  -3,  37), S(  -5,  37), S(   3,  27),
};

const Score KING_PSQT[32] = {
 S( -86, -89), S(  93, -28), S(  34,   3), S( -17,  -9),
 S(  -9,   6), S(  94,  43), S(  91,  38), S(  79,  23),
 S(  -6,  12), S( 130,  39), S( 113,  35), S(  17,  37),
 S( -41,   8), S(  94,  26), S(   9,  38), S( -24,  40),
 S( -92,   9), S(  21,  17), S(  30,  19), S( -19,  25),
 S( -13,  -5), S(  43,   7), S(  21,   8), S(   0,  10),
 S(   2, -10), S(   7,  12), S( -12,   8), S( -42,   5),
 S(   2, -47), S(   7, -11), S(  -3, -18), S( -16, -37),
};

const Score KNIGHT_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S( -25,  35), S(  14,  31), S(  46,  21), S(  37,  42),
 S(  16,  16), S(  37,  21), S(  31,  33), S(  48,  30),
 S(  14,  -2), S(  27,  19), S(  12,  24), S(  16,  33),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const Score BISHOP_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   5,  20), S(  31,  14), S(  54,  20), S(  52,  -3),
 S(  12,  12), S(  17,  21), S(  28,  11), S(  40,   5),
 S( -18,  24), S(  27,  12), S(  25,   6), S(  30,  16),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const Score KNIGHT_MOBILITIES[9] = {
 S( -40, -63), S( -14, -10), S(   3,  22), S(  13,  32),
 S(  23,  39), S(  30,  43), S(  36,  45), S(  44,  42),
 S(  50,  32),};

const Score BISHOP_MOBILITIES[14] = {
 S( -26, -31), S(  -6, -10), S(   7,   7), S(  15,  27),
 S(  26,  37), S(  33,  46), S(  35,  52), S(  36,  57),
 S(  38,  60), S(  39,  65), S(  45,  61), S(  66,  62),
 S(  69,  69), S(  66,  67),};

const Score ROOK_MOBILITIES[15] = {
 S( -52,-140), S( -18, -11), S(  -6,   8), S(   3,   6),
 S(  -1,  28), S(  -1,  37), S(  -4,  45), S(   0,  46),
 S(   6,  47), S(   9,  51), S(  13,  52), S(  13,  56),
 S(  17,  59), S(  23,  60), S(  33,  55),};

const Score QUEEN_MOBILITIES[28] = {
 S(-385,-407), S(-160,-108), S(  -4,-217), S( -10, -58),
 S(  -9,  32), S(  -4,  36), S(  -5,  60), S(  -2,  76),
 S(   0,  92), S(   4,  93), S(   6,  98), S(   7, 103),
 S(   7, 107), S(  10, 107), S(  10, 112), S(  14, 103),
 S(  11, 103), S(  13, 103), S(   6,  95), S(  16,  81),
 S(  24,  56), S(  38,  36), S(  51,  -3), S(  41, -11),
 S(  51, -42), S( 194,-149), S( 133,-194), S( -99, -75),
};

const Score KNIGHT_OUTPOST_REACHABLE = S(8, 17);

const Score BISHOP_OUTPOST_REACHABLE = S(7, 4);

const Score BISHOP_TRAPPED = S(-154, -104);
...
const Score DOUBLED_PAWN = S(-5, -28);
...
const Score CONNECTED_PAWN[8] = {
 S(   0,   0), S(  78,  39), S(  33,  29), S(  16,  10),
 S(   8,   3), S(   3,   1), S(   1,  -2), S(   0,   0),
};

const Score CANDIDATE_PASSER[8] = {
 S(   0,   0), S(   0,   0), S( -35, 160), S(  -7,  38),
 S( -14,  26), S( -20,   6), S( -32,   2), S(   0,   0),
};

const Score PASSED_PAWN[8] = {
 S(   0,   0), S(  41, 147), S(   9, 196), S(   3, 103),
 S( -12,  53), S(  -9,  30), S(  -4,  26), S(   0,   0),
};
...
const Score PASSED_PAWN_ADVANCE_DEFENDED = S(21, 23);

const Score KNIGHT_THREATS[6] = { S(2, 17), S(4, 8), S(39, 26), S(75, -6), S(44, -67), S(197, -3),};

const Score BISHOP_THREATS[6] = { S(4, 16), S(31, 38), S(13, 9), S(46, 6), S(48, 75), S(111, 85),};

const Score ROOK_THREATS[6] = { S(4, 24), S(32, 34), S(35, 41), S(-1, 21), S(79, 6), S(181, 11),};

const Score KING_THREATS[6] = { S(13, 27), S(24, 37), S(-7, 31), S(15, 23), S(-180, -326), S(0, 0),};

const Score PAWN_THREAT = S(63, 12);

const Score PAWN_PUSH_THREAT = S(19, 0);

const Score HANGING_THREAT = S(2, 11);

const Score PAWN_SHELTER[4][8] = {
 { S(-27, 3), S(40, 2), S(-29, 43), S(-7, 7), S(4, -6), S(24, -17), S(22, -35), S(0, 0),},
 { S(-41, 3), S(-43, 41), S(-5, 28), S(-26, 15), S(-16, 2), S(19, -13), S(24, -21), S(0, 0),},
 { S(-21, -7), S(-9, 26), S(-18, 25), S(-7, 4), S(-11, -1), S(-8, -1), S(23, -11), S(0, 0),},
 { S(-20, 4), S(15, 40), S(-53, 28), S(-20, 22), S(-12, 9), S(-9, 4), S(-10, 5), S(0, 0),},
};

const Score PAWN_STORM[4][8] = {
 { S(-24, -18), S(-12, -9), S(-11, -10), S(-19, 0), S(-39, 31), S(-2, 132), S(143, 168), S(0, 0),},
 { S(-12, -8), S(1, -11), S(2, -5), S(-7, 3), S(-19, 25), S(-111, 151), S(30, 205), S(0, 0),},
 { S(12, -10), S(20, -17), S(16, -9), S(8, -1), S(-14, 24), S(-125, 131), S(-94, 184), S(0, 0),},
 { S(4, -3), S(7, -21), S(13, -10), S(6, -14), S(-14, 10), S(-38, 95), S(-89, 161), S(0, 0),},
};

const Score BLOCKED_PAWN_STORM[8] = {
 S(-17, 12), S(51, -60), S(17, -34), S(16, -27), S(2, -18), S(-36, -20), S(0, 0), S(0, 0),
};

const Score KS_KING_FILE[4] = { S(11, -3), S(6, 1), S(0, 4), S(-33, -1),};
