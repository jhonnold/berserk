...
// clang-format off
const Score MATERIAL_VALUES[7] = { S(78, 134), S(342, 407), S(358, 416), S(468, 720), S(1103, 1214), S(   0,   0), S(   0,   0) };

const Score BISHOP_PAIR = S(23, 54);

const Score PAWN_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(  43, 171), S(  -2, 180), S(  48, 149), S(  70, 135),
 S(   5,  25), S(  -3,  18), S(  29, -18), S(  27, -31),
 S( -12,   3), S(   4, -10), S(  -3, -18), S(  16, -20),
 S( -16, -10), S( -10, -10), S(  -1, -17), S(   5, -10),
 S( -14, -14), S(   8, -15), S(  -3, -12), S(   7,  -7),
 S( -12, -11), S(   6, -12), S(  -7,  -9), S(   5,   2),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const Score KNIGHT_PSQT[32] = {
 S(-120, -65), S( -52, -27), S( -37, -15), S(  -7, -17),
 S( -13, -14), S( -16,  15), S(  23,  -6), S(  12,  -3),
 S(  15, -19), S(   4,  -1), S(  -5,  13), S(  24,  -8),
 S(  10,   3), S(   1,  -5), S(  12,   0), S(  19,  -2),
 S(  11,  27), S(  16,   3), S(  19,  22), S(  20,  25),
 S(  -7,  31), S(   7,  23), S(   2,  33), S(  18,  35),
 S(   3,  46), S(   8,  20), S(   4,  27), S(   9,  37),
 S( -20,  45), S(  -2,  31), S(   5,  27), S(   9,  38),
};

const Score BISHOP_PSQT[32] = {
 S( -30,  26), S( -18,  27), S( -48,  24), S( -72,  25),
 S(  -8,  18), S( -28,  35), S( -11,  33), S( -10,  28),
 S(  -8,  25), S( -22,  27), S( -16,  30), S(  -1,  15),
 S( -24,  24), S(   3,  20), S(  -7,  19), S(   8,  19),
 S(  15,   8), S(  -7,  15), S(   0,  22), S(   9,  16),
 S(   4,  15), S(  17,  24), S(   8,  30), S(   5,  35),
 S(  15,  14), S(  23,  20), S(  15,  16), S(   1,  35),
 S(  12,  17), S(  16,  18), S(   4,  32), S(   2,  30),
};

const Score ROOK_PSQT[32] = {
 S(   7,  36), S( -19,  47), S( -47,  62), S( -44,  62),
 S(  14,  25), S(   3,  38), S(  20,  35), S(  19,  29),
 S(   2,  30), S(  41,  26), S(  38,  31), S(  40,  22),
 S(  -5,  47), S(   7,  39), S(  24,  43), S(  21,  39),
 S( -11,  52), S(   4,  44), S(  -4,  54), S(   4,  56),
 S(  -9,  54), S(   8,  39), S(   8,  49), S(   6,  53),
 S( -14,  69), S(   1,  56), S(   9,  54), S(  11,  55),
 S(   0,  77), S(   0,  65), S(   3,  65), S(  10,  57),
};

const Score QUEEN_PSQT[32] = {
 S(  80, 168), S(  73, 186), S( 105, 178), S(  77, 203),
 S(  57, 186), S(  29, 221), S(  53, 242), S(  51, 222),
 S(  86, 163), S(  78, 200), S(  69, 214), S(  74, 219),
 S(  69, 202), S(  70, 218), S(  64, 218), S(  59, 232),
 S(  75, 190), S(  72, 212), S(  68, 212), S(  60, 230),
 S(  73, 183), S(  82, 180), S(  69, 202), S(  70, 195),
 S(  79, 154), S(  84, 140), S(  84, 154), S(  81, 172),
 S(  86, 134), S(  70, 152), S(  68, 156), S(  76, 148),
};

const Score KING_PSQT[32] = {
 S(-144, -97), S( 104, -40), S(  26,  15), S( -88,  12),
 S(   4,  -8), S( 141,  26), S( 144,  38), S( 175,  28),
 S(   7,  -4), S( 190,  15), S( 185,  33), S(  92,  42),
 S( -55,   5), S( 164,  16), S( 106,  45), S(  67,  57),
 S(-112,  15), S(  72,  24), S( 129,  36), S(  94,  48),
 S( -37,   0), S(  41,  22), S(  75,  33), S(  85,  39),
 S( -37, -24), S( -28,  11), S( -25,  36), S( -43,  47),
 S( -39, -71), S( -32, -37), S( -32,  -3), S( -34,  -4),
};

const Score KNIGHT_POST_PSQT[32] = {
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
 S( -17,  12), S(  24,  14), S(  51,  -3), S(  35,  35),
 S(  23,   8), S(  50,  31), S(  42,  36), S(  54,  33),
...
const Score KNIGHT_MOBILITIES[9] = {
 S( -45, -80), S( -23, -23), S( -10,  12), S(  -2,  24),
 S(   8,  31), S(  14,  38), S(  20,  40), S(  28,  36),
 S(  34,  28),};

const Score BISHOP_MOBILITIES[14] = {
 S( -36, -26), S( -18,  -6), S(  -6,   9), S(   1,  27),
 S(  11,  34), S(  17,  40), S(  20,  42), S(  22,  43),
 S(  25,  44), S(  25,  45), S(  32,  40), S(  48,  38),
 S(  49,  46), S(  33,  47),};

const Score ROOK_MOBILITIES[15] = {
 S( -51,-105), S( -19,  26), S(  -8,  44), S(   1,  31),
 S(  -2,  46), S(  -1,  55), S(  -3,  56), S(  -2,  63),
 S(   5,  60), S(   9,  59), S(  14,  56), S(  16,  49),
 S(  21,  46), S(  25,  42), S(  34,  32),};

const Score QUEEN_MOBILITIES[28] = {
 S(-291,-320), S( -76,  36), S(  54, -99), S(  47,  62),
 S(  51, 143), S(  53, 150), S(  51, 177), S(  55, 187),
 S(  56, 203), S(  60, 201), S(  61, 205), S(  63, 209),
 S(  63, 214), S(  65, 213), S(  66, 216), S(  69, 207),
 S(  65, 209), S(  65, 209), S(  64, 196), S(  66, 192),
 S(  77, 168), S(  95, 141), S( 112, 103), S(  99, 103),
 S( 105,  75), S( 198,   3), S( 235, -88), S(  -8,  53),
};
...
const Score BISHOP_TRAPPED = S(-148, -152);

const Score ROOK_TRAPPED = S(-43, -41);
...
const Score OPEN_ISOLATED_PAWN = S(-13, -14);

const Score BACKWARDS_PAWN = S(-8, -13);

const Score CONNECTED_PAWN[8] = {
 S(   0,   0), S(  89,  16), S(  30,  24), S(  12,  13),
 S(   6,   4), S(   2,   2), S(   0,  -3), S(   0,   0),
};

const Score PASSED_PAWN[8] = {
 S(   0,   0), S(  41, 162), S(  12, 216), S(   0, 120),
 S( -14,  62), S(  -8,  35), S(  -2,  30), S(   0,   0),
};
...
const Score BISHOP_THREATS[6] = { S(3, 11), S(28, 33), S(9, 7), S(44, 14), S(47, 77), S(84, 55),};

const Score ROOK_THREATS[6] = { S(5, 18), S(25, 34), S(30, 40), S(-7, 36), S(68, 21), S(123, 10),};

const Score KING_THREATS[6] = { S(40, 72), S(33, 82), S(4, 52), S(84, 31), S(-111, -293), S(0, 0),};
