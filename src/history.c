...
void UpdateEasyCapsCorrection(int raw, int real, int depth, Board* board, ThreadData* thread, SearchStack* ss) {
  if (ss->easyCaps) {
    const int16_t correction = Min(4096, Max(-4096, 4 * (real - raw) * depth));
    int16_t* easyCapCorrection =
      &thread->easyCapsCorrection[EasyCapsZobrist(board, ss->easyCaps) & EASY_CAPS_CORRECTION_MASK];
    AddHistoryHeuristic(easyCapCorrection, correction);
  }
}
