...
INLINE void DetermineDeltaAndApplySubAdd(acc_t* dest, acc_t* src, int f1, int f2, acc_t* delta) {
  ...
  for (size_t c = 0; c < N_HIDDEN / UNROLL; ++c) {
    ...
    for (size_t i = 0; i < NUM_REGS; i++)
      regi_store(&diff[i], regs[i]);

    for (size_t i = 0; i < NUM_REGS; i++)
      regs[i] = regi_add(regs[i], inputs[i]);
...
INLINE void DetermineDeltaAndApplySubAddSub(acc_t* dest, acc_t* src, int f1, int f2, int f3, acc_t* delta) {
  regi_t regs[NUM_REGS];
...
INLINE void DetermineDeltaAndApplySubAddSubAdd(acc_t* dest, acc_t* src, int f1, int f2, int f3, int f4, acc_t* delta) {
  regi_t regs[NUM_REGS];
...
INLINE void AddCachedDeltaSub(acc_t* dest, acc_t* src, int f1, acc_t* delta) {
  regi_t regs[NUM_REGS];
...
INLINE void AddCachedDeltaSubAdd(acc_t* dest, acc_t* src, int f1, int f2, acc_t* delta) {
  regi_t regs[NUM_REGS];
